...
// CODE HERE
hellobtnfunctionOn = () => {sayHelloButton.style.backgroundColor = 'red'; sayHelloButton.style.color = 'white'}
sayHelloButton.addEventListener('mouseover', hellobtnfunctionOn)
...
// CODE HERE
hellobtnfunctionOff = () => {sayHelloButton.style.backgroundColor = '#EFEFEF'; sayHelloButton.style.color = 'black'}
sayHelloButton.addEventListener('mouseout', hellobtnfunctionOff)
...
const ohMy = () => {
    // YOUR CODE HERE
    axios.get('http://localhost:3000/animals').then((res) => {for(i =0; i < res.data.length; i++) {
        console.log(res.data[i]);

        let pdata = document.createElement("p")
        pdata.textContent = res.data[i]
        document.querySelector("body").appendChild(pdata)
...
const repeatMyParam = (str) => {
    //YOUR CODE HERE
    axios.get(`http://localhost:3000/repeat/${str}`).then((res) => {console.log(res.data); let repeatTxt = document.querySelector("#repeat-text");
...
// CODE HERE
function getRequest() {
    axios.get('http://localhost:3000/query-test/?strightup!!!!').then((data) => {console.log(data); })
}
...
/*
7 and 6. 
how does the parameter work?? you cant pass it without it calling the function first before you click it. FIGURED IT OUT 

why do you pick your own query?? what data is the solution coming from and what hsould the console look like??  FIGURED IT OUT 
